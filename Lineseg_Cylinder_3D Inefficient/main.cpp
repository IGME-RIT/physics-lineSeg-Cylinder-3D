...
bool is_colliding(Line &l1, Cylinder &c1)
{
	...
	*/
	...
	d2 = c1.point2 - c1.point1;
	r = l1.point1 - c1.point1;
	...
	*/
	...
	if (s == 0.0f || s == 1.0f)
	{
		//When the nearest point is between the two planes containing the the circles at the end of the cylinder,
		//We find the projection of that point on the line joining the centers of the cylinder.
		d = glm::dot(p1 - c1.point1, d2);
	...
	if (t == 0.0f || t == 1.0f)
	{
		//If the closest point is near the end of the cylinder, we need to detect if the line intersects the the circle or not.

		glm::vec3 n = glm::normalize(glm::cross(d1, d2));
		glm::vec3 RadiusVec = glm::normalize(glm::cross(n, d2)) * c1.radius;
		...
		glm::vec3 diameter = pointB - pointA;
		AR = glm::normalize(glm::cross(pointA - l1.point1, diameter));
		BR = glm::normalize(glm::cross(diameter, pointA - l1.point2));

		//If both the end points of the line segment are on the same side of the circle,then the line segment is not intersecting the circle.
		if (glm::dot(AR, BR) < 0.0f)
			return false;						// Early exit. The line and the cylinder are not colliding.
		

		// get the plane contating the two lines, and find the two points on the edge of tcylinder(diameter).
		//If the end points of the diameter are on the same side of the line l1, then the line intersects cylinder
		AR = glm::normalize(glm::cross(pointA - l1.point1,d1));
		BR = glm::normalize(glm::cross(d1,pointB - l1.point1));

		if (glm::dot(AR, BR) >= 0.0f)
		{
			aaa = pointA;
			bbb = pointB;
			return true;			//early quit. the line intersects the cylinder 
		}
		else
			return false;			// Early quit. they do not intersect.
	...
	if (dis <= cylinder.radius)		// If the closest point is closer than the distance of a radius, the objects are colliding.
	{
		return true;
	}
	else
...
void renderScene()
{
	...
	// Set the uniform matrix in our shader to our MVP matrix for the line.
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "Line segment-Cylinder collision detection in 3D", nullptr, nullptr);
